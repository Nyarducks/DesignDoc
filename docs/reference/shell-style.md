...
Test runners may drop `-e` to collect failures — `set -u` still applies.
...
the flagged pattern is intentional, and always write the reason in a
comment on the line above the directive — a bare `disable=` reads as
"nobody checked this".
