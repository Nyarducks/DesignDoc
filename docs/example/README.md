...
|---|---|---|
| api | REST surface, event ingestion, auth, audit | [architecture/api/](architecture/api/) |
| web | Operator dashboard — map, timeline, alerts | [architecture/web/](architecture/web/) |
| worker | ETA computation, notification dispatch | [architecture/worker/](architecture/worker/) |
| infra | Cluster topology, scaling, observability | [architecture/infra/](architecture/infra/) |
...
- **Event-sourced shipment state** over a mutable status column —
  ...
  [architecture/api/shipments.md](architecture/api/shipments.md) for the surface
  this produces.
...
[architecture/infra/](architecture/infra/) for the observability model.
