...
description: Known problems and improvement backlog for Feedping — one NNNN-<slug>.md file per issue; closed ones move to archived/.
status: current
...
them. An issue that gets scheduled becomes a plan or a code change; set
`resolved_by:` in its frontmatter.
