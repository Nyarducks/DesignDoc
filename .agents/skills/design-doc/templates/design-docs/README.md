...
docs carry the living truth.

One `NNNN-<change>/` directory per change — numbered like ADRs and
issues so the listing orders them; a change that fits a single PR is a
lone `NNNN-<change>.md` in this directory.
...
- Skip the doc when the change is unambiguous — a bug fix, a mechanical
  rename, a config tweak. Write one when the change has a trade-off
  worth recording.
