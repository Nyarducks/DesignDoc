...
<!-- Why decisions need a durable home — e.g. they were previously recorded
only in PR bodies and commit messages, which are hard to discover and do
not survive squashed history. -->
...
(`type: ADR`). Design docs live under `docs/design-docs/` with the same
